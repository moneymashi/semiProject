...
public class MainController {
	...
	public String adminSearchList(@ModelAttribute("AUCTION_USER") User user,
			@ModelAttribute("USER_PAYMENT") Payment pay, @ModelAttribute("AUCTIONEER_INFO") Auctioneer acnr,
			@ModelAttribute("USER_MESSAGE1") Message msg, @ModelAttribute("USER_MESSAGE2") Message msg2, Model m) {
		/*
		 * m.addAttribute("adminSearchList", service.userAList(user));
		 * m.addAttribute("adminSearchList", service.paymentAList(pay,schId));
		 * m.addAttribute("adminSearchList",
		 * service.auctioneerAList(acnr,schId));
	...
	public static String randomNum(int loopcount) {
		String randomStr = "";
		int getNo = 0;
		for (int i = 1; i <= loopcount; i++) {
			Random rnd = new Random();
			getNo = rnd.nextInt(9);
			randomStr = randomStr + Integer.toString(getNo);
		}
		return randomStr;
	}
}
