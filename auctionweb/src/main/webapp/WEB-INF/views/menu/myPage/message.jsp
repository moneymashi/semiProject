...
<script type="text/javascript">
	$(function() {
		$("#messageBtn").click(
			function() {$("form").attr("action","${path }/auction/myPage/messageInsert.do");
				alert("메세지 전송");
				$("form").submit();
			});
		//수신함 클릭했을때.
		$(".receiveBox").closest("div").click(function() {
			if($(this).children("input[name=checkNo]").val() != 1){
				$(location).attr("href",
						"${path}/auction/myPage/messageReceiveCheck.do?message_id="+ $(this).children().val());
			}else{
				alert("읽은 메세지입니다.");
			}
		});
		...
		<c:if test="${receiveBox!='nothing' }">
			<c:forEach var="receiveBoxJspOnly" items="${receiveBox }">
				<div>
					...
					<input type="hidden" name="checkNo" value="${receiveBoxJspOnly.receive_check}"/>
					<div>receive_check :</div>
					<c:if test="${receiveBoxJspOnly.receive_check==1}">
						<div>읽음!!</div>
					</c:if>
					<c:if test="${receiveBoxJspOnly.receive_check!=1}">
						<div>읽지않음</div>
		...
		<c:if test="${sendBox!='nothing' }">
			<c:forEach var="sendBoxJspOnly" items="${sendBox }">
				<div>
					...
					<div>receive_check :</div>
					<c:if test="${sendBoxJspOnly.receive_check==1}">
						<div>읽음!!</div>
					</c:if>
					<c:if test="${sendBoxJspOnly.receive_check!=1}">
						<div>읽지않음</div>
